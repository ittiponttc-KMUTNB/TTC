...
/* ══ Name tag bob: follows horse up/down ══ */
@keyframes name-bob {
...
@keyframes dust {
    ...
    60%  { opacity:.3;  transform:scale(1.3) translateX(-16px); }
...
/* ══ Track ══ */
.track-container {
    background: linear-gradient(180deg, #174d17 0%, #2d7b2d 50%, #174d17 100%);
    border-radius: 18px;
...
.track-container::before {
    ...
    background: repeating-linear-gradient(
        90deg, transparent 0px, transparent 39px,
...
.track-lane {
    ...
    position:relative;
    min-height:72px;   /* taller to give name-tag space above */
}
.leader-lane {
    background:rgba(255,215,0,.07) !important;
    border:1px solid rgba(255,215,0,.35) !important;
    animation:leader-glow 1.8s ease-in-out infinite;
}

.lane-rank  { font-family:'Kanit',sans-serif; font-size:1.3rem; font-weight:900;
...
/* ══ Horse wrapper: moves left together with name ══ */
.horse-wrap {
    position:absolute;
    top:50%;
    /* left set inline */
...
/* ══ Floating name tag above horse ══ */
.horse-name-tag {
    position:absolute;
    bottom:calc(100% + 4px);   /* sits above the emoji */
    left:50%;
    ...
    padding:2px 8px;
    pointer-events:none;
    animation:name-bob var(--spd,.38s) ease-in-out infinite;
}
...
.horse-emoji {
    ...
    animation:gallop var(--spd,.38s) ease-in-out infinite;
...
.dust {
    ...
    font-size:.7rem;
    animation:dust .48s ease-out infinite;
    pointer-events:none; z-index:5;
}

.finish-flag {
    position:absolute; right:12px; top:50%;
    font-size:1.3rem; opacity:.65;
    animation:flag-wave 1.3s ease-in-out infinite;
}

.score-badge {
    font-family:'Kanit',sans-serif; font-size:1.05rem; font-weight:700;
    background:rgba(240,192,64,.12); border:1px solid rgba(240,192,64,.4);
...
.race-title { font-family:'Kanit',sans-serif; font-size:2.6rem; font-weight:900;
              color:#f0c040; text-align:center;
              text-shadow:0 0 30px rgba(240,192,64,.5);
              letter-spacing:2px; margin-bottom:.1rem; }
.race-subtitle { font-family:'Sarabun',sans-serif; font-size:1.05rem;
                 color:#7ab3d4; text-align:center; margin-bottom:1.6rem; }
.section-header { font-family:'Kanit',sans-serif; font-size:1.15rem; font-weight:700;
                  color:#f0c040; border-bottom:2px solid rgba(240,192,64,.3);
                  padding-bottom:7px; margin-bottom:14px; }
.stButton>button { font-family:'Kanit',sans-serif !important; font-weight:700 !important;
                   border-radius:10px !important; border:none !important;
                   padding:.55rem 1.6rem !important; font-size:1rem !important;
...
# ─── Build Track ──────────────────────────────────────────────────────────────
def build_track(df_sorted: pd.DataFrame, progress: dict) -> str:
    lanes = ""
    for i, (_, row) in enumerate(df_sorted.iterrows()):
        ...
        left      = max(4, min(86, pct * 0.82 + 4))   # 4% – 86%
        ...
        dust     = DUST_EMOJIS [i % len(DUST_EMOJIS)]
        ldr_cls  = "leader-lane" if rank == 1 else ""
        spd      = "0.30s" if rank == 1 else f"{0.36 + i*0.025:.3f}s"

        lanes += f"""
        <div class="track-lane {ldr_cls}">
            <div class="lane-rank {rank_cls}">{rank_sym}</div>
            <div class="lane-track">
                <!-- horse wrapper: left position transitions smoothly -->
                <div class="horse-wrap" style="left:{left:.1f}%; --spd:{spd};">
                    <div class="horse-name-tag" style="--spd:{spd};">{name}</div>
                    <div class="horse-emoji">{horse}</div>
                </div>
                <!-- dust behind horse -->
                <div class="dust"
                     style="left:{dust_left:.1f}%;
                            animation-delay:{i*0.08:.2f}s;">{dust}</div>
...
if st.session_state.animate and len(df_sorted) > 0:
    ...
    for step in range(STEPS + 1):
        ...
        track_ph.markdown(build_track(df_sorted, curr), unsafe_allow_html=True)
        time.sleep(0.03)
    ...
    if len(df_sorted) > 0:
        ...
        track_ph.markdown(build_track(df_sorted, pos), unsafe_allow_html=True)
    else:
        ...
        if ri < len(top3):
            ...
            with cols[ci]:
                ...
                <div style="text-align:center;background:rgba(255,255,255,.05);
                    ...
                    <div style="font-size:2.3rem;display:inline-block;
